...
let messageList = {}
try {
  islogin = localStorage.islogin || islogin
  self = JSON.parse(localStorage.self) || self
  friends = JSON.parse(localStorage.friends) || friends
  messageList = JSON.parse(localStorage.messageList) || messageList
} catch ($e) {}
...
let state = {
  ...
  headerTitle: 'Message',
  // 初始化基础数据
  data: { self: self, friends: friends },
  // ajax请求数据是否结束
  isAjax: true,
